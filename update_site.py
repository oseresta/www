...
STRATEGY_DESCRIPTIONS = {
    ...
    "dma_bo": {
        ...
        "description": "An <b>Accumulation Strategy</b> that scales investment aggressively as prices drop below a <b>Dynamic Moving Average</b> (optimized for market state). Includes a <b>Sentiment Filter</b> (VADER) to suppress buys during negative news cycles. Note: This analysis is performed on the <b>top 50 stocks</b> from Nifty 50 holdings. <b>Baseline</b> refers to the standard Buy & Hold strategy return for the same period. Forward Testing serves as a real-time validation mechanism that is updated daily. It operates on the strict assumption that every buy signal results in a trade executed at the daily closing price. This ensures that the performance metrics reflect a realistic and consistent execution model, free from look-ahead bias, by treating every signal as a definitive action taken at the market close."
    },
    "dma_hmm": {
...
<div class="w3-bar w3-black w3-large">
  <a href="index.html" class="w3-bar-item w3-button"><i class="fa-solid fa-home"></i> Home</a>
...
<head>
    ...
    <style>
        ...
        /* Signal Colors */
        .signal-Buy, .signal-BUY { color: green; font-weight: bold; }
        .signal-Sell, .signal-SELL { color: red; font-weight: bold; }
        .signal-Hold, .signal-HOLD { color: gray; }
        ...
        /* Force X icon color */
        .fa-x-twitter { color: #ffffff !important; }
    </style>
</head>
<body class="w3-light-grey">
...
<div class="w3-bar w3-top w3-black w3-large" style="z-index:4; display: flex; align-items: center; padding: 0 10px;">
  <div style="display: flex; align-items: center;">
      ...
      <button class="w3-button w3-hover-none w3-hover-text-light-grey share-btn" onclick="shareTo('facebook')" title="Share on Facebook"><i class="fa-brands fa-facebook"></i></button>
...
<nav class="w3-sidebar w3-collapse w3-white w3-animate-left" style="z-index:3;width:260px;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block" id="nav-container">
    <a href="#" class="w3-bar-item w3-button w3-padding-16 w3-hide-large w3-dark-grey w3-hover-black" onclick="w3_close()" title="close menu"><i class="fa-solid fa-xmark fa-fw"></i>  Close Menu</a>
...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
  ...
  <div id="placeholder-msg" class="w3-container w3-padding-32 w3-center">
      <h3><i class="fa-solid fa-arrow-left"></i> Reports are updated every day. Click a date (last 10 days) in the sidebar to view reports.</h3>
  </div>
    ...
    <div class="w3-modal-content w3-animate-zoom w3-card-4" style="max-width:600px">
      ...
      <div class="w3-container w3-padding-large">
        ...
        <div class="w3-center w3-padding-16" style="background:#f1f1f1; border: 1px solid #ddd;">
            ...
            <a href="https://docs.google.com/forms/d/e/1FAIpQLScd_IjFV5jOAmzE0RArcp-laGisCG-vnXnEV7k28cFQsleU5g/viewform?usp=sf_link" target="_blank" class="w3-button w3-black"><i class="fa-solid fa-up-right-from-square"></i> Open in New Tab</a>
        </div>
      </div>
...
function renderSidebar() {
    ...
    container.innerHTML += '<a href="#" class="w3-bar-item w3-button w3-padding-16 w3-hide-large w3-dark-grey w3-hover-black" onclick="w3_close()" title="close menu"><i class="fa-solid fa-xmark fa-fw"></i> Close Menu</a>';
    ...
    for (const [key, strategy] of Object.entries(manifest)) {
        ...
        btn.innerHTML = `<i class="fa-solid fa-chart-line fa-fw"></i> ${strategy.name} <i class="fa-solid fa-caret-down"></i>`;
        ...
        strategy.dates.forEach(item => {
            ...
            link.innerHTML = `<i class="fa-solid fa-calendar fa-fw"></i> ${item.date}`;
            link.onclick = (e) => {
                e.preventDefault();
                loadReport(key, item);
...
function shareTo(platform) {
    ...
    const text = "Check out this stock analysis report: " + document.title;
    let shareUrl = "";
...
function jsonToTable(data, strategyKey) {
    ...
    const excluded = ["Model", "Strategy", "Best_Window", "Metric_Value", "Metric_Type", "Sentiment_Score", "timestamp"];
        ...
        if (strategyKey === 'pv') {
            excluded.push("performance", "volatility");
        }
        
        // Get headers, filtering out excluded ones
        const allHeaders = Object.keys(data[0]);
        let headers = allHeaders.filter(h => !excluded.includes(h));
        
        // Header mapping for Peak Valley (US)
        const pvHeaderMap = {
            'ticker': 'Ticker',
        ...
        if (strategyKey === 'pv') {
            headers = ['ticker', 'current_price', 'rsi', 'macd', 'trend', 'mtf_trend', 'adx', 'divergence', 'stops', 'action', 'reasons'];
        }
        ...
        headers.forEach(h => {
            let displayHeader = h;
            if (strategyKey === 'pv' && pvHeaderMap[h]) {
                displayHeader = pvHeaderMap[h];
            } else {
                // Replace underscores with spaces and Capitalize Each Word
                displayHeader = h.replace(/_/g, " ").replace(/\b\w/g, l => l.toUpperCase());
                // Specific overrides
            ...
            let classList = "";
            if (strategyKey === 'pv') {
                const alwaysVisible = ['ticker', 'current_price', 'stops', 'action'];
                if (!alwaysVisible.includes(h.toLowerCase())) {
                    classList = "class='w3-hide-small'";
                }
            }
        ...
        data.forEach(row => {
            ...
            // Handle Signal coloring (generic)
            if (h === 'Signal' && val) {
                val = `<span class="signal-${val}">${val}</span>`;
            }
            // Handle Action coloring for PV
            if (strategyKey === 'pv' && h.toLowerCase() === 'action' && val) {
                const signalClass = val.split(' ')[0].replace(/[^a-zA-Z]/g, ''); // Extract BUY from BUY (VOL BLOCKED)
                val = `<span class="signal-${signalClass}">${val}</span>`;
            }
...
async function loadReport(strategyKey, dateItem) {
    ...
    // Load Backward Images (Always check)
    ...
    // Load Forward Images (Always check)
